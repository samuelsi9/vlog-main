...
class _LoginPageState extends State<LoginPage> with TickerProviderStateMixin {
  ...
  // ── Helper: show / hide the overlay ────────────────────────────────────────
  void _setLoading(bool value, {String message = 'Signing in…'}) {
    if (!mounted) return;
    setState(() {
      _isLoading = value;
  ...
  void signUserIn() async {
    ...
    _setLoading(true, message: 'Signing in…'); // ← show overlay
    ...
    try {
      ...
      if (data.isNotEmpty && data['user'] != null) {
        ...
        _setLoading(false); // ← hide overlay on failure
        if (!mounted) return;
      ...
      _setLoading(false); // ← hide overlay on error
      if (!mounted) return;
  ...
  Future<void> _signInWithGoogle() async {
    _setLoading(true, message: 'Connecting with Google…'); // ← show overlay
    ...
    try {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (_) => MainScreen(token: userId, showWelcomeOverlay: true),
        ),
      );
    } on GoogleSignInException catch (e) {
      _setLoading(false); // ← hide overlay
  ...
  Future<void> _signInWithApple() async {
    _setLoading(true, message: 'Connecting with Apple…'); // ← show overlay
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text('Apple registration failed: ${e.message}'),
            backgroundColor: Colors.red),
      );
    } catch (e) {
      _setLoading(false); // ← hide overlay
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text('Apple registration failed: ${e.toString()}'),
            backgroundColor: Colors.red),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // ── Wrap body in Stack so the overlay sits on top ──────────────────────
      body: Stack(
        children: [
                            ...
                            children: [
                              TextFormField(
                                ...
                                decoration: InputDecoration(
                                  ...
                                  hintStyle: TextStyle(
                                      color: _lightGrey, fontSize: 15),
                                  prefixIcon: Icon(Icons.email_outlined,
                                      color: _lightGrey, size: 22),
                                  filled: true,
...
InkWell(
  onTap: () {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => MainScreen(token: null, showWelcomeOverlay: true),
      ),
    );
  },
  child: Center(
    child: Text(
      ...
      style: TextStyle(
        ...
        decorationColor: Colors.grey,          // makes underline gray
        decorationThickness: 1.5,              // slightly thinner for subtle look
        ...
        fontWeight: FontWeight.w500,
          ...
          // ── NEW: Loading overlay — appears on top of everything ─────────────
          if (_isLoading)
            Positioned.fill(
...
//         context,
//         MaterialPageRoute(
//           builder: (_) => MainScreen(token: userId, showWelcomeOverlay: true) //hasPhone
